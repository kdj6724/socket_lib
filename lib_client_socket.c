...
static void* lib_send_thread(void *data) {
	...
	while(1) {
		...
		if (readLen <= 0) {
			...
			write(info->socketFd, buffer, readLen);
...
int lib_client_socket_init(struct SocketLibinfo* info, int ip, int port) {
...
int lib_client_socket_run(struct SocketLibinfo* info) {
	int res = -1;
	int status;
	pthread_attr_t attr;
...
void lib_client_socket_stop(struct SocketLibinfo* info) {
	int status;
